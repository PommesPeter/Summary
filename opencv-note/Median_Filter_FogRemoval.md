...
> 以下引用自：[https://zhuanlan.zhihu.com/p/99195839](https://zhuanlan.zhihu.com/p/99195839)
...
其中$I(x)$为输入图像,需要求去雾后的输出图像$J(x)$,所以我们只要计算出全局大气光值$A$和$t(x)$

 1. 定义$F(x)=A(1-t(x))$，焦作大气光幕或者雾浓度。
 2. 计算$$M(x)=\mathop{min}\limits_{c\in\lbrace r,g,b\rbrace}(I(x))$$即是求暗通道
 ...
 6. 通过式子$$J(x,y)=\frac{I(x)-F(x)}{1-\frac{F(x)}{A}}$$获得去雾后的图像，这个式子就是把原始子移项变形得到的。

自此，算法结束，得到了利用中值滤波实现的去雾后的结果。
